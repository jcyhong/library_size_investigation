...
show_plot <- function(otu, sample_table, sample_id_col, col, method = "TSS", category, interest, log = FALSE, write = FALSE) {
  ...
  sample_df <- sample_df[sample_df[col]!="NA",]
  sample_df <- sample_df[sample_df[col]!="None",]
  
  krus_pval = kruskal.test(sample_df$lib_sizes ~ sample_df[,col])$p.value
  if (write) {
    result <- data.frame(study_id = STUDY_ID, summary=SUMMARY, method = method, group = col, p_val = krus_pval, category=category, interest=interest)
